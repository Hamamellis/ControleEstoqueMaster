                    ...
                    using (var transacao = conexao.BeginTransaction())
                    {
                        foreach (var produtoInventario in dados)
                        {
                            using (var comando = new SqlCommand())
                            {
                                comando.Connection = conexao;
                                comando.Transaction = transacao;
                                comando.CommandText = "insert into inventario_estoque (data, id_produto, quant_estoque, quant_inventario, " +
                                    "motivo) values (@data, @id_produto, @quant_estoque, @quant_inventario, @motivo)";
